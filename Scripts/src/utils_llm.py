...
def get_deepseek_chat_model(model_name="deepseek-chat"):
    model = OpenAI(api_key=os.environ.get('DEEPSEEK_API_KEY'), base_url='https://api.deepseek.com')
    return NamedModel(model, model_name)

def prompt_deepseek_chat(model, prompt, image_path=None, demonstrations=None):
    """
    Generates a response from the GPT-4o model using optional image input,
    and supports adding demonstration examples.
    ...
    try:
        ...
        raise RuntimeError(f"An error occurred while querying the DeepSeekChat model: {e}")
...
def get_commercial_model(model_name):
    model_map = {
        ...
        "deepseek-chat": get_deepseek_chat_model,
    }
...
def prompt_commercial_model(client, model_name, prompt, image_id, demonstrations=None):
    ...
    prompt_map = {
        ...
        "deepseek-chat": prompt_deepseek_chat,
    }
...
if __name__ == "__main__":
    ...
    # client = get_deepseek_chat_model()
    # response = prompt_deepseek_chat(client, text_prompt, "")
    # print("\nDeepseek Response:")
    # print(response)
